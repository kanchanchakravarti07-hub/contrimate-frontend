...
const History = () => {
  ...
  const fetchExpenses = (userId) => {
    fetch(`http://localhost:8081/api/expenses/user/${userId}`)
      .then(res => res.json())
      .then(data => {
  ...
  const parseDate = (dateInput) => {
      // If null, return 1970 (Sinks to bottom)
      if (!dateInput) return new Date(0);
      
      // Handle Java Array [2026, 2, 8, 14, 30]
      if (Array.isArray(dateInput)) {
          const [year, month, day, hour, minute, second] = dateInput;
          return new Date(year, month - 1, day, hour || 0, minute || 0, second || 0);
      }
      
      // Handle String "2026-02-08T..."
  ...
  return (
    // 🔥 Background changed to Deep Slate (#0f172a)
          ...
          {expenses.length > 0 ? expenses.map((expense) => {
             ...
             return (
                <div key={expense.id} className="card" style={{ 
                    padding: '16px', borderRadius: '20px', display: 'flex', alignItems: 'center', 
                    justifyContent: 'space-between', background: '#1e293b', border: '1px solid #334155' 
                }}>
                  
                  <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    {/* Icon Box */}
                    <div style={{ 
                        width: '48px', height: '48px', background: '#0f172a', borderRadius: '14px', 
