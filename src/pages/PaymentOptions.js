...
const PaymentOptions = () => {
  ...
  const { friendId, friendName, amount: initialAmount } =
    location.state || { amount: 0, friendName: 'User' };
  ...
  const handleFinalSettle = async (appName) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/expenses/settle`, {
        method: 'POST',
  ...
  return (
    <div className="container" style={{ maxWidth: '450px', margin: '0 auto', paddingBottom: '40px' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
          ...
          {isEditing ? (
            ...
            <h1
              ...
              style={{ fontSize: '56px', color: 'white', margin: 0, fontWeight: '900', cursor: 'pointer' }}
            >
              {editableAmount}
            </h1>
          )}
        ...
        {isEditing && (
          <p style={{ fontSize: '12px', color: 'white', marginTop: '10px', opacity: 0.8 }}>
            Tap outside to save
          </p>
        )}
      </div>
